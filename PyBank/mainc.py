...
print("Welcome to the Bank Analysis Software")
print()
...
with open(file_name, newline='') as csvfile:
     ...
     for i in months:
          ...
          total_revenue = total_revenue + int(revenue[index])

          if (index > 0):
               rev_change = int(revenue[index]) - int(revenue[index-1])
     ...
     for i in rev_change_list:
          total_rev_change = total_rev_change + int(rev_change_list[index])
     
          if(int(rev_change_list[index]) >= max_change):
              max_change = int(rev_change_list[index])
              max_index = index

          elif(int(rev_change_list[index]) <= min_change):
               min_change = int(rev_change_list[index])
               min_index = index

          index = index + 1

     average_change_rev = int(total_rev_change/index)
